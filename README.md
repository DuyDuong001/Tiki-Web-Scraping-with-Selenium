...
##################################################################  REQUIRED PARTS  #####################################################################
    ...
    try:
        ...
        if info['product_url'].find("https")== -1:    # The find() method returns -1 if the value is not found.
...
####################################################################  EXTRA PARTS  ###################################################################### 
    ...
    try: 
        freeship = product_item.find_element_by_css_selector('div.thumbnail > img').get_attribute('src')  #  get element
    ...
    try: 
        ...
        info['star_percent']= star_location[-5:-1]    ### Number + %  , e.g. 86%
...
def check_no_of_page(no_of_page): 
    last_page_url = main_cat['URL']+'?page='+str(no_of_page)     # Example about this: https://tiki.vn/nha-cua-doi-song/c1883?page=2
...
df = pd.DataFrame(data=data_list[2], columns=data_list[2][0].keys()) #Print list by page 1,2,3...
